import { SharedOptions, getTasks, runTaskListOnce } from "graphile-worker";
...
export const getJobs = async (
  ...
  const { rows } = await asRoot(client, () =>
    client.query(
      "select j.* from graphile_worker._private_jobs j join graphile_worker._private_tasks t on j.task_id = t.id where $1::text is null or t.identifier = $1::text order by j.id asc",
      [taskIdentifier],
    ),
  );
  return rows;
