...
COPY packages/portable-file/ /app/packages/portable-file/
RUN yarn workspace @repo/portable-file build

COPY packages/base-types/ /app/packages/base-types/
RUN yarn workspace @repo/base-types build
...
COPY packages/ai-chat/ /app/packages/ai-chat/
RUN yarn workspace @repo/ai-chat build
...
RUN yarn workspace @repo/homepage build
...
FROM node:24.15.0-alpine3.23 AS clean
# Import our shared args
ARG NODE_ENV
ARG ROOT_URL
...
COPY --from=builder-client /app/apps/homepage/package.json /app/apps/homepage/package.json
COPY --from=builder-client /app/apps/homepage/dist/ /app/apps/homepage/dist/
...
COPY --from=builder-plugin /app/plugins/bible/package.json /app/plugins/bible/
